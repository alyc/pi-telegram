...
function readConfig(ctx: ExtensionContext): TelegramConfig {
  const projectConfigFilePath = join(ctx.cwd, ".pi", "telegram.json");
  try {
    const content = existsSync(projectConfigFilePath)
...
function writeConfig(ctx: ExtensionContext, config: TelegramConfig): void {
  const projectPath = join(ctx.cwd, ".pi", "telegram.json");
  const configContent = JSON.stringify(config, null, "\t") + "\n";
  if (existsSync(projectPath)) {
    writeFileSync(projectPath, configContent, "utf8");
  } else {
...
export default function (pi: ExtensionAPI) {
  ...
  async function dispatchAuthorizedTelegramMessages(
    ...
    if (lower === "/help" || lower === "/start") {
      ...
      if (config.allowedUserIds === undefined && firstMessage.from) {
        config.allowedUserIds = [firstMessage.from.id];
        writeConfig(ctx, config);
        updateStatus(ctx);
      }
      return;
  ...
  async function handleUpdate(
    ...
    if (config.allowedUserIds === undefined) {
      config.allowedUserIds = [message.from.id];
      writeConfig(ctx, config);
      updateStatus(ctx);
      await sendTextReply(
        message.chat.id,
  ...
  pi.registerCommand("telegram-status", {
    ...
    handler: async (_args, ctx) => {
      const status = [
        ...
        `allowed users: ${config.allowedUserIds?.join(", ") ?? "not paired"}`,
